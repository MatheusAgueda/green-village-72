...
export function createStage({canvas=null,width=1280,height=720,pixelRatio=1}={}){
 ...
 const generator=new THREE.PMREMGenerator(renderer),room=new RoomEnvironment(),environment=generator.fromScene(room,.08);scene.environment=environment.texture;scene.environmentIntensity=.35;room.dispose();generator.dispose();
 ...
 function fitBox(box,direction=new THREE.Vector3(1,.74,1.3),padding=1.2){const size=box.getSize(new THREE.Vector3()),centre=box.getCenter(new THREE.Vector3()),fov=THREE.MathUtils.degToRad(camera.fov),radius=size.length()/2;const distance=padding*Math.max(radius/Math.sin(fov/2),radius/(Math.sin(fov/2)*camera.aspect));camera.zoom=1;camera.position.copy(centre).add(direction.normalize().multiplyScalar(distance));camera.near=.05;camera.far=200;camera.lookAt(centre);camera.updateProjectionMatrix();return centre;}
 function preset(name,modelBounds){const b=modelBounds||new THREE.Box3(new THREE.Vector3(-3.11,-.18,-5.9),new THREE.Vector3(3.11,2.6,5.9)),c=b.getCenter(new THREE.Vector3()),s=b.getSize(new THREE.Vector3());camera.zoom=1;camera.up.set(0,1,0);
  ...
  if(name==='inside'){camera.position.set(8.3,13.3,13.7);camera.lookAt(0,0,0);if(camera.aspect<1)camera.position.multiplyScalar(1/camera.aspect*.9);return new THREE.Vector3(0,.3,0);}
  ...
  return fitBox(b,dir,1.02);
 }
 return {renderer,scene,camera,lighting,resize,preset,fitBox,render:()=>renderer.render(scene,camera),dispose(){environment.dispose();ground.geometry.dispose();ground.material.dispose();renderer.dispose();}};
}
