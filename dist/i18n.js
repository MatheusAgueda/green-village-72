const STRINGS={
  ...
  '72 m² é a designação comercial. As plantas indicam 11,80 × 6,22 m, equivalentes a um rectângulo exterior de 73,396 m². A área útil certificada não consta dos anexos; as áreas interiores apresentadas são estimativas.':'72 m² is the commercial designation. The plans show 11.80 × 6.22 m, equivalent to an exterior rectangle of 73.396 m². The certified useful area is not in the annexes; the interior areas shown are estimates.',
  'Guarde o resumo em PDF com planta, materiais e referências. A Green Village poderá confirmar a compatibilidade, os equipamentos incluídos, transporte, instalação e preço final na proposta comercial.':'Save the summary as PDF with layout, materials and references. Green Village can confirm compatibility, included equipment, transport, installation and final price in the commercial proposal.',
  ...
  '72 m² é a designação comercial. As plantas indicam 11,80 × 6,22 m, equivalentes a um rectângulo exterior de 73,396 m². A área útil certificada não consta dos anexos; as áreas interiores apresentadas são estimativas.':'72 m² es la designación comercial. Los planos indican 11,80 × 6,22 m, equivalentes a un rectángulo exterior de 73,396 m². El área útil certificada no consta en los anexos; las áreas interiores son estimaciones.',
  'Guarde o resumo em PDF com planta, materiais e referências. A Green Village poderá confirmar a compatibilidade, os equipamentos incluídos, transporte, instalação e preço final na proposta comercial.':'Guarde el resumen en PDF con plano, materiales y referencias. Green Village podrá confirmar la compatibilidad, los equipos incluidos, transporte, instalación y precio final en la propuesta comercial.',
...
function translate(root=document){
  const map=STRINGS[language];
  if(!map)return;
  for(const node of collectTextNodes(root)){
    if(node.parentElement?.closest('script,style,code'))continue;
    if(!ptOriginals.has(node))ptOriginals.set(node,node.nodeValue);
    const original=ptOriginals.get(node);
    const key=original.trim();
    if(map[key])node.nodeValue=original.replace(key,map[key]);
  }
}

function restorePT(root=document){
  for(const node of collectTextNodes(root)){
    if(ptOriginals.has(node))node.nodeValue=ptOriginals.get(node);
  }
}
...
export function initI18n(){
  ...
  const observer=new MutationObserver(mutations=>{
    ...
    for(const m of mutations)for(const node of m.addedNodes){
      ...
      else if(node.nodeType===Node.TEXT_NODE&&!ptOriginals.has(node)){
        ptOriginals.set(node,node.nodeValue);
        const map=STRINGS[language];
        if(map){const key=node.nodeValue.trim();if(map[key])node.nodeValue=node.nodeValue.replace(key,map[key]);}
      }
    }
  });
