...
export const DEFAULT_CONFIG=Object.freeze({schemaVersion:2,layout:'t2',exteriorId:white.id,exterior:MATERIALS.find(m=>m.id===white.id).previewHexApprox,floorId:DATA.swatches['floor-spc'][1].id,floor:MATERIALS.find(m=>m.id===DATA.swatches['floor-spc'][1].id).previewHexApprox,interior:'#eeede8',interiorName:'Branco natural',kitchen:'linear',kitchenRef:'kitchen-01',bathroom:'standard',bathroomRef:'bathroom-01',bathroomUV:null,roof:false,porch:false,lighting:'neutral'});
...
export function validateConfiguration(value){
 ...
 const result={};for(const key of Object.keys(DEFAULT_CONFIG)){
  const v=data[key];if(v===undefined)throw new Error('Falta o campo '+key+'.');result[key]=v;
 }
 const enumField=(k,values)=>{if(!values.includes(result[k]))throw new Error('Opção inválida: '+k+'.');};
 enumField('layout',DATA.layouts.map(x=>x.id));enumField('kitchen',['none','linear','l','island']);enumField('bathroom',['standard','mirrored']);enumField('lighting',['neutral','exterior']);enumField('kitchenRef',DATA.kitchens.map(x=>x.id));enumField('bathroomRef',DATA.bathrooms.map(x=>x.id));
...
export function summaryRows(s){const find=id=>SWATCHES.find(x=>x.id===id),p=DATA.layouts.find(p=>p.id===s.layout);return [['Modelo','Expandível 72 · 40 pés'],['Planta',p.label],['Exterior',find(s.exteriorId)?.label||'Cor livre '+s.exterior],['Paredes interiores',s.interiorName],['Pavimento SPC',find(s.floorId)?.label||'Cor livre '+s.floor],['Cozinha',({none:'Sem cozinha representada',linear:'Linear',l:'Em L',island:'Com ilha'})[s.kitchen]],['Referência de cozinha',s.kitchenRef.replace('kitchen-','')+' · inspiração fotográfica, sem cotas'],['Casa de banho',s.bathroom==='standard'?'Distribuição da planta (aproximada)':'Alternativa espelhada (proposta)'],['Revestimento UV',find(s.bathroomUV)?.label||'Por definir'],['Ambiente de banho',s.bathroomRef.replace('bathroom-','')+' · referência visual'],['Telhado adicional',s.roof?'Seleccionado · catálogo 2 830 €':'Não seleccionado'],['Alpendre',s.porch?'Seleccionado · catálogo 2 050 €':'Não seleccionado']];}
